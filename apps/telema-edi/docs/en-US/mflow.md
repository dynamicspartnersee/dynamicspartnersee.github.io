...
| **Issue E-Order** | Yes or No | Yes, if you also want to send the order to the vendor; No, if only to MatchFlow. |
| **Issue E-Receipt** | Yes or No | Yes, if you also want to send the receipt to the vendor; No, if only to MatchFlow. |
...
If the vendor card has both **Telema mFlow** and **Issue E-Order** enabled, two orders will be issued — one for MatchFlow and one for the vendor.

## Sending a Purchase Receipt
The purchase receipt is issued automatically during posting.

If the vendor card has both **Telema mFlow** and **Issue E-Receipt** enabled, two receipts will be issued — one for MatchFlow and one for the vendor.
...
BC automatically updates the linked purchase order during the posting.
